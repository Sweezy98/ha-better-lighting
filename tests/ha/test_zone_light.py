"""Zone light entity: aggregation, relative dimming, and on-state memory."""
...
async def _setup(
    hass: HomeAssistant, one: MemberLight, two: MemberLight, **zone_kwargs
):
    await setup_members(hass, [one, two])
    entry = hub_entry(**zone_kwargs) if zone_kwargs else hub_entry()
...
async def _call(hass: HomeAssistant, service: str, **data) -> None:
    await hass.services.async_call(
        "light", service, {"entity_id": ZONE, **data}, blocking=True
    )
    await hass.async_block_till_done()


async def test_creates_one_entity_per_zone(hass: HomeAssistant) -> None:
    await _setup(
        hass,
...
async def test_brightness_averages_only_lit_members(hass: HomeAssistant) -> None:
    ...
    assert hass.states.get(ZONE).attributes["brightness"] == 200


async def test_any_member_on_means_the_zone_is_on(hass: HomeAssistant) -> None:
    await _setup(
        hass,
        MemberLight("One", is_on=True, brightness=100),
        MemberLight("Two", is_on=False),
...
async def test_zone_reports_member_colour_capabilities(hass: HomeAssistant) -> None:
    await _setup(
        hass,
        MemberLight("One", is_on=True, brightness=100),
        MemberLight("Two", is_on=True, brightness=100),
    )
    modes = hass.states.get(ZONE).attributes["supported_color_modes"]
...
async def test_only_previously_on_lights_come_back(hass: HomeAssistant) -> None:
    ...
    # light.two was already off before the zone was switched off, so it stays off.
    assert hass.states.get("light.two").state == "off"
...
async def test_relative_dim_moves_members_by_their_own_headroom(
    ...
    await _setup(hass, one, two)
    assert hass.states.get(ZONE).attributes["brightness"] == 125
