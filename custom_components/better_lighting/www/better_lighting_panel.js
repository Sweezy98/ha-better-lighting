...
class BetterLightingPanel extends HTMLElement {
  ...
  async _load() {
    const { rooms } = await this._call("config");
    this._rooms = rooms;
  ...
  _paint() {
    ...
    rooms.innerHTML = `<h2>Rooms</h2><ul>${this._rooms
      .map(
        (room) =>
          `<li data-room="${room.id}" aria-selected="${
            room.id === this._roomId
          }">${room.name}<div class="muted">${room.scenes.length} scenes</div></li>`
      )
      .join("")}</ul>`;
    rooms.querySelectorAll("li").forEach((item) =>
    ...
    this._scene ? this._paintEditor() : this._paintScenes();
  }
